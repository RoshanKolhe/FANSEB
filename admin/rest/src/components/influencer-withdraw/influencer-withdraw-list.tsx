...
import { Shop, SortOrder, User } from '@/types';
  ...
  let columns:any = [
    {
      ...
      key: 'user',
      align: alignLeft,
      render: function Render(user: any) {
    ...
    columns.push({
      ...
      render: (id: string) => {
        ...
        if (hasAccess(adminOnly, permissions)) {
          return (
            <ActionButtons
              detailsUrl={`${Routes.influencerWithdraw.list}/${id}`}
              id={id}
            />
          );
        }
...
export default InfluencerWithdrawList;
